...
An $M$-dimensional random vector $x \in \mathbb{R}^M$ follows a multivariate
Gaussian distribution if
\[
x \sim \mathcal{N}(\mu, \Sigma),
\]
where $\mu \in \mathbb{R}^M$ is the mean and $\Sigma \in \mathbb{R}^{M \times M}$
...
If $y = A x + b$, where $A$ is a matrix and $b$ a vector, then
\[
y \sim \mathcal{N}(A\mu + b,\ \, A\Sigma A^\top).
\]

\subsection{Partitioned Form, Marginals, Conditionals}
Let
\[
...
The product of two Gaussians (up to normalization) is Gaussian:
\[
\mathcal{N}(x \mid \mu_1,\Sigma_1)\,
\mathcal{N}(x \mid \mu_2,\Sigma_2)
\propto
...
\begin{enumerate}
    \item Compute Cholesky: $\Sigma = LL^\top$.
    \item Draw $z \sim \mathcal{N}(0, I)$.
    \item Set $x = \mu + Lz$.
\end{enumerate}
...
\subsection{Key Facts to Remember}
\begin{itemize}
    \item Level sets are ellipsoids: 
    $(x-\mu)^\top \Sigma^{-1}(x-\mu)=c$.
    \item Closed under linear transformations.
    \item Closed under marginalization.
    \item Closed under conditioning.
    \item Product of Gaussians is Gaussian.
    \item Gaussian prior + Gaussian likelihood gives Gaussian posterior.
\end{itemize}
% ============================================================
